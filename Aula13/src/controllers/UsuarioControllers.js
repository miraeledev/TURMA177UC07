    ...
    static async login(req, res) {
        try {
            ...
            const token = jwt.sign(
                { id: usuario.id, email: usuario.email, nome: usuario.nome }, //Informações que ficarão no payload do Token
...
static perfil(req, res) {
    try {
        const usuario = req.usuario; //Dados do usuário extraídos do token pelo middleware
        if(!usuario){
            res.status(404).json({msg: "Nenhum dado do usuário para exibir"});
            return;
        }
        res.status(200).json({msg: "Dados encontrados", usuario: usuario});

} catch (error) {
        res.status(500).json({ msg: "Erro interno ao exibir os dados do user", error: error.message });
}
}};
